...
class Statistics < BaseStatistics
  def initialize(wager_condition = nil)
    @intervals = [50, 60, 70, 80, 90, 100].map { |band| [band, Interval.new(band)] }.to_h
    ...
    sql = <<~SQL
      ...
      COUNT(*) sample_size,
      CAST(SUM(correct) AS FLOAT) / COUNT(*) accuracy
      FROM
      (SELECT
      CASE
      ...
      END rel_conf,
      CASE
        WHEN (r.confidence >= 50 AND j.outcome = TRUE) OR
           (r.confidence < 50 AND j.outcome = FALSE) THEN 1
        ELSE 0
      END correct
      FROM responses r
      INNER JOIN predictions p ON r.prediction_id = p.id
      INNER JOIN (
        SELECT prediction_id, MAX(id) id
        FROM judgements
        GROUP BY prediction_id
      ) mrj ON mrj.prediction_id = p.id
    ...
    records.each do |datum|
      @intervals[datum['band'].to_i].update(datum.values)
    end
  end
